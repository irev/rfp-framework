...
Before publication, check unique code and monotonic version; known capabilities and valid capability configuration; registered handler when specified; active document/workflow/accounting/integration references; reachable workflow terminal paths; resolvable actors and return targets; coherent return/resubmit; valid API/event/schema contracts; unknown-key rejection; definition hash; tenant/authorization negative tests; and ERP mapping/UAT where applicable. Record reviewer, effective date, rollout scope, rollback choice, and monitoring owner. An active definition is never edited; changes create another version. Requests keep their pinned definitions and historical snapshots unless a controlled migration is approved.
